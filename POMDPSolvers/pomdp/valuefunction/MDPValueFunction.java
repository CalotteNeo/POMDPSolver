...
public class MDPValueFunction extends PolicyStrategy {
	...
	/*
	 * public static MDPValueFunction getInstance(){ return g_vMDP; }
	 * 
	 * public static MDPValueFunction getInstance( POMDP pomdp, double
	 * dExplorationRate ){ if( g_vMDP == null ) g_vMDP = new MDPValueFunction(
	 * pomdp, dExplorationRate ); return g_vMDP; }
	 */
	...
	/*
	 * public double getQValue( BeliefState bs, int iAction ){ int iState = 0;
	 * Iterator itStates = bs.getNonZeroEntries(); double dPr = 0.0, dValue =
	 * 0.0, dSum = 0.0; Map.Entry e = null;
	 * 
	 * while( itStates.hasNext() ){ e = (Map.Entry)itStates.next(); iState =
	 * ((Integer) e.getKey()).intValue(); dPr = ((Double)
	 * e.getValue()).doubleValue(); dValue = m_adQValues[iState][iAction]; dSum
	 * += dPr * dValue; } return dSum; }
	 */
	...
	protected void makeVectors(boolean bFullQFunction) {
		...
		if (bFullQFunction) {
			/*
			 * for( int iState : getValidStates() ){ Logger.getInstance().log(
			 * iState + " - " ); for( iAction = 0 ; iAction < m_cActions ;
			 * iAction++ ){ dQValue = getQValue( iState, iAction ) ;
			...
			if (m_ivBestActions != null) {
				for (int iState : getValidStates()) {
					...
					// Logger.getInstance().logln( iState + ", " + getStateName(
					// iState ) + " => " + iAction );
			...
			Logger.getInstance().logFull("MDPVF", 0, "makeVectors",
					"Done creating policy, |S| = " + m_cStates + ", vertex count = " + m_avBestActions.countEntries());
			// Logger.getInstance().logFull( "MDPVF", 0, "makeVectors", "Done
			// creating policy" );
		}
		// improveVectors( 100 );
	}
	...
	protected void improveVectors(int cMaxIterations) {
		...
		for (iIteration = 0; iIteration < cMaxIterations; iIteration++) {
			...
			for (iAction = 0; iAction < m_cActions; iAction++) {
				/*
				 * avNext = new SparseAlphaVector( null, m_cStates, m_cActions,
				 * m_cObservations, m_fTransition, m_fObservation, iAction );
				 */
	...
	public double computeValueFunction(int cMaxIterations, double dEpsilon, boolean bFixedPolicy) {
		...
		double dDelta = 0.0, dMaxDelta = 1000.0, dMaxValue = 0.0, dMinValue = 0.0, dValue = 0.0,
				dMaxValueForAllStates = Double.NEGATIVE_INFINITY;
		int iAction = 0, iMaxAction = 0;
		...
		for (iIteration = 0; (iIteration < cMaxIterations) && (dMaxDelta > dEpsilon); iIteration++) {
			...
			for (int iState : getValidStates()) {
				if (!m_pPOMDP.isTerminalState(iState)) {
					 ...
					 * Logger.getInstance().logln( "V(" + iState + ") = " +
					 * m_adValues.elementAt( iState ) ); for( iAction = 0 ;
					 * iAction < m_cActions ; iAction++ ){ for( int i = 0 ; i <
					 * m_cStates ; i++ ){ if( tr( iState, iAction, i ) > 0.0 )
					 * if( m_adValues.elementAt( i ) > 0.0 ) dDelta =
					 * updateState( iState ); } } }
					 */
				}
				if ((iState % 100000 == 0) && (iState > 0))
					Logger.getInstance().logFull("MDPVI", 0, "ComputeV",
							"Iteration " + iIteration + " done " + iState + " states");
				if (dDelta > dMaxDelta)
					dMaxDelta = dDelta;
				dValue = getValue(iState);
				if (dValue > dMaxValue) {
			...
			Logger.getInstance().logFull("MDPVF", 0, "computeValueFunction",
					"After " + iIteration + " iterations, delta = " + dMaxDelta + " min " + iMinState + " = "
							+ dMinValue + " max " + iMaxState + " = " + dMaxValue);
		}
	...
	/**
	 * Attempts to load the value function from a file. If the value function
	 * file is not found it computes the value function and saves it to a file.
	 */
	public synchronized void valueIteration(int cMaxIterations, double dEpsilon) {
		...
		if (!m_bLoaded) {
			...
			Logger.getInstance().logFull("MDPVF", 0, "VI",
					"MDP value iteration done - iterations " + cMDPBackups + " delta " + dMaxDelta);
	...
	public void load(String sFileName) throws Exception {
		...
		if ((cStates != m_cStates) || (cActions != m_cActions))
			throw new Exception("Unmatching state or action count. Expected <" + m_cStates + "," + m_cActions
					+ "> found <" + cStates + "," + cActions + ">");
		...
		if (sType.equals("ValueFunction")) {
			...
			// Logger.getInstance().logFull( "MDPVF", 0, "load", "Done loading
			// best actions" );
			Logger.getInstance().logFull("MDPVF", 0, "load", "Done loading best actions, |S| = " + m_cStates
					+ ", vertex count = " + m_avBestActions.countEntries());
		} else {
	...
	protected void save(String sFileName) throws Exception {
		...
		if (m_bFullQFunction) {
			...
			docValueFunction.appendChild(eMDPFunction);
			/*
			 * for( int iState : getValidStates() ){ eState =
			 * docValueFunction.createElement( "State" ); iAction =
			 * (int)m_avBestActions.valueAt( iState ); //iAction =
			 * (int)m_ivBestActions.elementAt( iState ); eState.setAttribute(
			 * "Id", iState + "" ); eState.setAttribute( "Action", iAction + ""
			 * ); eMDPFunction.appendChild( eState ); }
			 */
	 ...
	 * @author zbng
	 */
	public int getWeightedAction(BeliefState bs) {
		int iAction = 0, iMaxAction = 0;
		double dMaxWeightedValue = Double.MAX_VALUE * -1, dQValue = 0.0;
		...
		for (iAction = 0; iAction < m_cActions; iAction++) {
			double dWeightedValue = 0.0;
			dWeightedValue = getQValue(bs, iAction);
			if (dWeightedValue > dMaxWeightedValue) {
				dMaxWeightedValue = dWeightedValue;
				iMaxAction = iAction;
			}
		}
		return iMaxAction;
	...
	public Number[] importanceSampling(int iAction, BeliefState bsCurrent) {
		...
		for (iObservation = 0; iObservation < m_cObservations; iObservation++) {
			...
			if (dPr > 0) {
				...
				// dValue = ( getValue( bsNext ) + m_pPOMDP.R( bsCurrent,
				// iAction, bsNext ) ) * dPr + 0.1;
				dValue = dPr;
				if (dValue < dMinValue)
					dMinValue = dValue;
				dSumValues += dValue;
	...
	public Number[] importanceSamplingForStartState() {
		...
		Iterator<Entry<Integer, Double>> itStartState = m_pPOMDP.getBeliefStateFactory().getInitialBeliefState()
				.getNonZeroEntries().iterator();
