...
public class CreateBeliefSpaces {
	...
	public static Vector<BeliefState> createRandomSpace(POMDP pomdp, int cBeliefPoints) {
		...
		// Logger.getInstance().log( "CreateBeliefSpaces", 0,
		// "createRandomSpace", "The maximal ADR observed over " + cBeliefPoints
		// + " points is " + dMaxADR );
		return vPoints;
	}
	...
	public static Vector<BeliefState> computeOutlyingBeliefPoints(POMDP pomdp, int cPoints) {
		...
		for (iPoint = 0; iPoint < cPoints; iPoint++) {
			...
			if (iPoint > 0 && iPoint % 10 == 0) {
				...
				Logger.getInstance().logln(iPoint + ")" + " |B| = " + vBeliefPoints.size() + " distances " + sDistances
						+ " avg preds = " + dAvg + " max preds " + cMaxPreds +
						// " max R = " + dMaxR +
						" |B| = " + pomdp.getBeliefStateFactory().getBeliefStateCount() + " max memory "
						+ rtRuntime.maxMemory() / 1000000 + " actual memory "
						+ (rtRuntime.totalMemory() - rtRuntime.freeMemory()) / 1000000 + " free memory "
						+ rtRuntime.freeMemory() / 1000000);
				sDistances = "";
			}
		}
	...
	public static Vector<BeliefState> createHeuristicSpace(POMDP pomdp, int iSeed, int cBeliefPoints) {
		...
		/*
		 * for( BeliefState bsCurrent : vPoints ){ Logger.getInstance().logln(
		 * bsCurrent ); Logger.getInstance().log( "Successors: " ); for(
		 * BeliefState bsSucc : bsCurrent.getSuccessors() ){
		 * Logger.getInstance().log( " bs" + bsSucc.getId() + ", " ); }
		 * Logger.getInstance().logln(); Logger.getInstance().log(
		 * "Predecessors: " ); for( BeliefState bsPred :
		 * bsCurrent.getPredecessors() ){ Logger.getInstance().log( " bs" +
		 * bsPred.getId() + ", " ); } Logger.getInstance().logln(); }
		 */

		Logger.getInstance().log("CreateBeliefSpaces", 0, "createHeuristicSpace",
				"The maximal ADR observed over " + cBeliefPoints + " points is " + dMaxADR);
		return vPoints;
	}

	// protected static void createRandomSpaces( POMDP pomdp, String sPath, int
	// cSpaces, int cBeliefPoints ) throws IOException, TransformerException,
	// ParserConfigurationException{
	// double dDiscountedReward = 0.0, dSumDiscountedReward = 0.0,
	// dMaxDiscountedReward = 0.0, dMinDiscountedReward = 1000.0;
	// int cTrials = 0;
	...
	// /*
	// ValueIteration vfHSVI = new HeuristicSearchValueIteration(
	// pomdp.m_fTransition, pomdp.m_fReward,
	...
	// Logger.getInstance().logln( "Creating belief space with random seed = " +
	// iSeed );
	// while( pomdp.getBeliefStateFactory().getBeliefStateCount() <
	// cBeliefPoints ){
	// //pomdp.simulate( cBeliefPoints, 4, -1 );
	// dDiscountedReward = pomdp.computeMDPDiscountedReward( 50, pvQMDP, false,
	// null );
	// //dDiscountedReward = pomdp.computeDiscountedReward( 50, pvQMDP, null,
	// true, aiActionCount );
	...
	// ){
	// Logger.getInstance().logln( "Collected " +
	// pomdp.getBeliefStateFactory().getBeliefStateCount() +
	...
	// }
	//
	// Logger.getInstance().logln( "Collected discounted reward = " +
	// dSumDiscountedReward / cTrials +
	// " over " + cTrials + " trials " +
	// " max = " + dMaxDiscountedReward +
	// " min = " + dMinDiscountedReward );
	//
	// pomdp.getBeliefStateFactory().saveBeliefSpace( sPath + "/" +
	// pomdp.getName() + "/BeliefSpace" + iSeed +/* "-" + cBeliefPoints +*/
	// ".xml" );
	...
	// " actual memory " + ( rtRuntime.totalMemory() - rtRuntime.freeMemory() )
	// / 1000000 +
	...
	protected static void createIncreasingSizeRandomSpaces(POMDP pomdp, String sPath, int iInitialSize, int iFinalSize,
			int iIncrementSize, int cDuplicateSpaces, boolean bRandom)
			throws IOException, TransformerException, ParserConfigurationException {
		double dDiscountedReward = 0.0, dSumDiscountedReward = 0.0, dMaxDiscountedReward = 0.0,
				dMinDiscountedReward = 1000.0;
		int cTrials = 0;
		int cReportPoints = 1000;
		Runtime rtRuntime = Runtime.getRuntime();
		...
		/*
		 * ValueIteration vfHSVI = new HeuristicSearchValueIteration(
		 * pomdp.m_fTransition, pomdp.m_fReward, pomdp.m_fObservation,
		 * pomdp.m_cStates, pomdp.m_cActions, pomdp.m_cObservations,
		 * pomdp.m_dGamma, 0.01 );
		 */
		pvQMDP.valueIteration(20000, 0.0001);
		// vfHSVI.valueIteration( null, 25, 0.01, pomdp, 18.0 );
		...
		while (cBeliefPoints <= iFinalSize) {
			for (int iSeed = 0; iSeed < cDuplicateSpaces; iSeed++) {
				...
				dMinDiscountedReward = 1000.0;
				Logger.getInstance()
						.logln("Creating belief space of size " + cBeliefPoints + " with random seed = " + iSeed);
				while (pomdp.getBeliefStateFactory().getBeliefStateCount() < cBeliefPoints) {
					// pomdp.simulate( cBeliefPoints, 4, -1 );
					if (bRandom) {
						dDiscountedReward = pomdp.computeDiscountedReward(
								cBeliefPoints - pomdp.getBeliefStateFactory().getBeliefStateCount(), pvRandom, null,
								true, aiActionCount);
					} else {
						dDiscountedReward = pomdp.computeMDPDiscountedReward(500, pvQMDP, true, null);
					}
					// dDiscountedReward = pomdp.computeDiscountedReward( 50,
					// pvQMDP, null, true, aiActionCount );
					dSumDiscountedReward += dDiscountedReward;
					cTrials++;
					if (dDiscountedReward > dMaxDiscountedReward)
						dMaxDiscountedReward = dDiscountedReward;
					...
					if (pomdp.getBeliefStateFactory().getBeliefStateCount() >= cReportPoints) {
						Logger.getInstance().logln("Collected " + pomdp.getBeliefStateFactory().getBeliefStateCount()
								+ " points, discounted reward = " + dSumDiscountedReward / cTrials + " over " + cTrials
								+ " trials " + " max = " + dMaxDiscountedReward + " min = " + dMinDiscountedReward);
						cReportPoints += 10;
					}

				}

				Logger.getInstance().logln("Collected discounted reward = " + dSumDiscountedReward / cTrials + " over "
						+ cTrials + " trials " + " max = " + dMaxDiscountedReward + " min = " + dMinDiscountedReward);

				if (bRandom) {
					pomdp.getBeliefStateFactory().saveBeliefSpace(
							sPath + "/" + pomdp.getName() + "/RandomBeliefSpace" + iBeliefSpace + ".xml");
				} else {
					pomdp.getBeliefStateFactory()
							.saveBeliefSpace(sPath + "/" + pomdp.getName() + "/BeliefSpace" + iBeliefSpace + ".xml");
				}
				iBeliefSpace++;

				Logger.getInstance()
						.logln("Done seed = " + iSeed + " |B| = " + pomdp.getBeliefStateFactory().getBeliefStateCount()
								+ " max memory " + rtRuntime.maxMemory() / 1000000 + " actual memory "
								+ (rtRuntime.totalMemory() - rtRuntime.freeMemory()) / 1000000 + " free memory "
								+ rtRuntime.freeMemory() / 1000000);
				pomdp.getBeliefStateFactory().clear();
				Logger.getInstance()
						.logln("After memory release " + " max memory " + rtRuntime.maxMemory() / 1000000
	...
	protected static void createOutlyingSpace(POMDP pomdp, String sPath, String sModelName, int cBeliefPoints)
			throws IOException, TransformerException, ParserConfigurationException {
		Runtime rtRuntime = Runtime.getRuntime();
		...
		pomdp.getBeliefStateFactory().saveBeliefPoints(
				sPath + "/" + sModelName + "/OutlyingBeliefSpace-L2-" + cBeliefPoints + ".xml", vBeliefPoints);
		Logger.getInstance()
				.logln("Done " + " |B| = " + pomdp.getBeliefStateFactory().getBeliefStateCount() + " max memory "
						+ rtRuntime.maxMemory() / 1000000 + " actual memory "
						+ (rtRuntime.totalMemory() - rtRuntime.freeMemory()) / 1000000 + " free memory "
						+ rtRuntime.freeMemory() / 1000000);
		pomdp.getBeliefStateFactory().clear();
		Logger.getInstance()
				.logln("After memory release " + " max memory " + rtRuntime.maxMemory() / 1000000 + " actual memory "
						+ (rtRuntime.totalMemory() - rtRuntime.freeMemory()) / 1000000 + " free memory "
						+ rtRuntime.freeMemory() / 1000000);
	}
	...
	public static void main(String[] args) {
		...
		try {
			// MDPValueFunction.setPersistanceFile( sPath + "/Models/" +
			// sModelName + "QMDP.xml" );
			...
			err.printStackTrace();
			Logger.getInstance().logln(" free memory " + rtRuntime.freeMemory() / 1000000 + " total memory "
					+ rtRuntime.totalMemory() / 1000000 + " max memory " + rtRuntime.maxMemory() / 1000000 + "");
		}
	}
}
