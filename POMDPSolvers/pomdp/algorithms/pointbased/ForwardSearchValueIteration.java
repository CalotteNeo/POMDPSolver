...
public class ForwardSearchValueIteration extends ValueIteration {
	...
	public void valueIteration(int cMaxSteps, double dEpsilon, double dTargetValue, int maxRunningTime,
		...
		// public void valueIteration( int cMaxSteps, double dEpsilon, double
		// dTargetValue ){
		int iIteration = 0;
		boolean bDone = false;
		...
		for (iIteration = 0; (iIteration < cMaxSteps) && !bDone; iIteration++) {
			...
			if (((m_lCPUTimeTotal / 1000000000) >= 5) && (iIteration >= 10) && (iIteration % 5 == 0)
				...
				sMsg = "FSVI: Iteration " + iIteration + " |Vn| = " + m_vValueFunction.size() + " simulated ADR "
						...
						+ rtRuntime.totalMemory() / 1000000 + " free " + rtRuntime.freeMemory() / 1000000 + " max "
						+ rtRuntime.maxMemory() / 1000000 + "";
				Logger.getInstance().log("FSVI", 0, "VI", sMsg);
			} else {
				...
				sMsg = "FSVI: Iteration " + iIteration + " |Vn| = " + m_vValueFunction.size() + " time "
