...
public class GenericValueIteration extends ValueIteration {
	...
	public GenericValueIteration(POMDP pomdp, String collectionName, String backupOrderingName, int numBeliefs,
			int backupIterations, boolean useBlindPolicy, boolean bAllowDuplicates, boolean bReversedBackupOrder,
			boolean bUseFIB) {

		super(pomdp);
		collection = BeliefCollectionFactory.getBeliefCollectionAlgorithm(collectionName, this, bAllowDuplicates,
				bUseFIB);
	...
	public void valueIteration(int cIterations, double dEpsilon, double dTargetValue, int maxRunningTime,
			int numEvaluations) {
		...
		try {
			...
			PrintStream ps = new PrintStream(new FileOutputStream("logs/Basic/" + sModelName + "." + collection + "."
					+ ordering + "." + numBeliefsPerStep + "." + numBackupIterations + "." + ordering.isReversed() + "."
					+ numBackupIterations + "." + collection.allowDuplicates() + "." + (m_bUseFIB ? "FIB" : "QMDP")
					+ "." + (useBlindPolicy ? "BlindPolicy" : "MinPolicy") + "." + "txt", true));
			ps.println(collection + "\t" + ordering);
			int currentEvaluation = 1;

			// with numEvaluations evaluations, we find out how much time
			// alloted per evaluation
			...
			/*
			 * store the amount of time required for each segment of the
			 * algorithm
			 */
			...
			for (int i = 0; i < cIterations && !bDone; i++) {
				...
				/*
				 * removing duplicates here is problematic because we can't
				 * control it using the flag. On the other hand, if we move it
				 * elsewhere, we have to do so BEFORE the backup and newest
				 * points backup will not have b_0 and will behave horribly.
				 * Currently just commenting this out.
				 * 
				 * Collection<BeliefState> cleaned = new
				 * LinkedHashSet<BeliefState>(beliefPoints);
				 * beliefPoints.clear(); beliefPoints.addAll(cleaned);
				 */
				...
				if (totalTimeInSeconds > currentTarget) {
					// we will use the value function from the previous
					// iteration for testing. This way, we
					...
					Logger.getInstance().log("POMDPSolver", 0, "main",
							"ADR = " + dDiscountedReward + ", time = " + totalTimeInSeconds + "\n");
					// this is to capture all possible time intervals we may
					// have missed

					if (totalTimeInSeconds > currentTarget) {
						while ((currentTarget < totalTimeInSeconds) && (currentTarget <= maxRunningTime)) {
							// 计算线性插值
							double dInterpolatedValue = computeLinearInterpolation(totalTimeInSeconds,
									previousTotalTime, dDiscountedReward, dPreviousReward, currentTarget);
							ps.println(currentTarget + "\t" + previousTotalTime + "\t" + dInterpolatedValue + "\t"
									+ dPreviousReward);

							// System.out.println(currentTarget + "\t" +
							// previousTotalTime + "\t" + dInterpolatedValue +
							// "\t" + dPreviousReward);
				...
				Logger.getInstance()
						.logln("Iteration " + i + " |Vn| = " + m_vValueFunction.size() + " |B| = " + beliefPoints.size()
								+ " Delta = " + round(dDelta, 4) + " Time " + (lCurrentTime - lStartTime) / 1000000
								+ " Initial Belief Time " + initalBeliefTime / 1000000 + " Belief Collection Time "
								+ collectionTime / 1000000 + " Update Time " + updateTime / 1000000 + " #backups: "
								+ m_cBackups + " #dot product: " + AlphaVector.dotProductCount() + " |BS| "
								+ m_pPOMDP.getBeliefStateFactory().getBeliefStateCount() + " memory: " + " total: "
								+ rtRuntime.totalMemory() / 1000000 + " free: " + rtRuntime.freeMemory() / 1000000
								+ " max: " + rtRuntime.maxMemory() / 1000000 + "");

			}
			Logger.getInstance().logln("Finished " + getName() + " - Model : " + m_pPOMDP.getName() + " - time : "
					+ totalTimeInSeconds + " |BS| = " + beliefPoints.size() + " |V| = " + m_vValueFunction.size()
					+ " backups = " + m_cBackups + " GComputations = " + AlphaVector.getGComputationsCount() +

					" Initial Belief Time " + initalBeliefTime / 1000000 + " Belief Collection Time "
					+ collectionTime / 1000000 + " Update Time " + updateTime / 1000000
