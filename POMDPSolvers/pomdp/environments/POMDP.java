...
public class POMDP implements Serializable {
	...
	public POMDP() {
		...
		m_bGBasedBackup = false;// tau based doesn't work right now - need to
								// fix a bug there
	...
	public int observe(int iAction, int iState) {
		...
		if (iObservation == m_cObservations)
			throw new Error("Corrupted observation function - O( " + getActionName(iAction) + ", "
					+ getStateName(iState) + ", * ) = 0");
		return iObservation;
	}
	...
	public double computeAverageDiscountedReward(int cTests, int cMaxStepsToGoal, PolicyStrategy policy) {
		return computeAverageDiscountedReward(cTests, cMaxStepsToGoal, policy, true,
				ExecutionProperties.useHighLevelMultiThread() || ExecutionProperties.useMultiThread());
		// return computeAverageDiscountedRewardImportanceSampling( cTests,
		// cMaxStepsToGoal, policy, true,
		// ExecutionProperties.useHighLevelMultiThread() ||
		// ExecutionProperties.useMultiThread() );
		// return computeAverageDiscountedRewardParticleFiltering( cTests,
		// cMaxStepsToGoal, policy, true,
		// ExecutionProperties.useHighLevelMultiThread() ||
		// ExecutionProperties.useMultiThread() );
		// return
		// computeAverageDiscountedRewardParticleFilteringImportanceSampling(
		// cTests, cMaxStepsToGoal, policy, true,
		// ExecutionProperties.useHighLevelMultiThread() ||
		// ExecutionProperties.useMultiThread() );
	}
	 ...
	 * @param policy
	 * @param bOutputMessages
	 * @param bUseMultiThread
	 * @return
	 */
	public double computeAverageDiscountedReward(int cTests, int cMaxStepsToGoal, PolicyStrategy policy,
			boolean bOutputMessages, boolean bUseMultiThread) {
		...
		if (bUseMultiThread) {
			...
			for (iThread = 0; iThread < cThreads; iThread++) {
				aTrials[iThread] = new ComputeDiscountedReward(this, cMaxStepsToGoal, vValueFunction,
						cTests / cThreads);
		...
		if (bOutputMessages) {
			...
			Logger.getInstance().logln();
			Logger.getInstance().log("POMDP", 0, "computeAverageDiscountedReward",
					"After " + iTest + " tests. ADR " + round(dADR, 3) + ", stdev " + round(dStdev, 5) + " SE "
							+ round(dStandardError, 5) + " time " + lTotalCPU / 1.0 + "\n");
		}
	...
	public double computeAverageDiscountedRewardParticleFiltering(int cTests, int cMaxStepsToGoal,
			PolicyStrategy policy, boolean bOutputMessages, boolean bUseMultiThread) {
		countStatistics(false);
		...
		for (iTest = 0; (iTest < cTests) && (dStandardError > 0.05 * dADR); iTest++) {
			...
			// dDiscountedReward = computeDiscountedRewardI( cMaxStepsToGoal,
			// policy, null, false, null );
			dSumSquares += (dDiscountedReward * dDiscountedReward);
			dSumDiscountedRewards += dDiscountedReward;
			...
			// ", " + Math.abs( dDiscountedReward - dPreviousDiscountedReward )
			// );
		...
		if (bOutputMessages) {
			...
			Logger.getInstance().log("POMDP", 0, "computeAverageDiscountedReward",
					"After " + iTest + " tests. ADR " + round(dADR, 3) + ", stdev " + round(dStdev, 5) + " SE "
							+ round(dStandardError, 5) + " time " + lTotalCPU);
		}
	...
	public double computeAverageDiscountedRewardParticleFilteringImportanceSampling(int cTests, int cMaxStepsToGoal,
			PolicyStrategy policy, boolean bOutputMessages, boolean bUseMultiThread) {
		countStatistics(false);
		...
		for (iTest = 0; (iTest < cTests) && (dStandardError > 0.01 * dADR); iTest++) {
			dDiscountedReward = computeDiscountedRewardParticleFilteringImportanceSampling(cMaxStepsToGoal, policy,
					cParticles);
			dSumSquares += (dDiscountedReward * dDiscountedReward);
			dSumDiscountedRewards += dDiscountedReward;
			...
			if (iTest >= 10 && iTest % 10 == 0) {
				...
				lTotalCPU = (lEndCPU - lStartCPU) / 1000000000;
				Logger.getInstance()
						.logln(iTest + ") " + dADR + ", " + dStdev + ", " + dStandardError + " time " + lTotalCPU);
			}
		}
	...
	protected double computeMDPAverageDiscountedReward(int cTests, int cMaxStepsToGoal) {
		...
		String sInfo = "Status: #tests = " + cTests + ": " + " Time: " + cCurrentMilliseconds + " Reward: "
				+ dSumDiscountedRewards / cTests + g_sNewline;
	...
	protected double computeDiscountedReward(int cMaxStepsToGoal, PolicyStrategy policy,
			Vector<BeliefState> vObservedBeliefPoints, int[] aiActionCount) {
		return computeDiscountedReward(cMaxStepsToGoal, policy, vObservedBeliefPoints, false, aiActionCount);
	}

	/**
	 * Execute a single trial for computing discounted reward (DR). Simulates
	...
	// private int x = 0;
	public double computeDiscountedReward(int cMaxStepsToGoal, PolicyStrategy policy,
			Vector<BeliefState> vObservedBeliefPoints, boolean bExplore, int[] aiActionCount) {
		// if( x++ % 2 == 0 )
		// return computeDiscountedRewardI( cMaxStepsToGoal, policy,
	...
	public double computeDiscountedRewardWithParticleFiltering(int cMaxStepsToGoal, PolicyStrategy policy,
			int cMaxBeliefs) {
	...
	public double computeDiscountedRewardWithParticleFilteringImportanceSampling(int cMaxStepsToGoal,
			PolicyStrategy policy, int cMaxBeliefs) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		double dCurrentStepDiscountedReward = 0.0, dSumWeights = 0.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		BeliefState bsCurrentBelief = null, bsNext = null;
	...
	private Vector<Pair<BeliefState, Double>> reduceBeliefSet(Vector<Pair<BeliefState, double[]>> vNextBeliefs,
			int cMaxSamples, BeliefStateComparator comp) {
		...
		for (Pair<BeliefState, double[]> pSample : vSample) {
			...
			// dValue = adBeliefWeights[0];// / adBeliefWeights[2]; //w_t =
			// w_{t-1} * p(x_t) / h(x_t)
	...
	private Vector<Pair<BeliefState, Double>> reduceBeliefSet(Vector<Pair<BeliefState, Double>> vNextBeliefs,
			int cMaxSamples, PolicyStrategy policy, BeliefStateComparator comp) {
	...
	public double computeDiscountedRewardI(int cMaxStepsToGoal, PolicyStrategy policy,
			Vector<BeliefState> vObservedBeliefPoints, boolean bExplore, int[] aiActionCount) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		...
		return dDiscountedReward;// + dDiscountFactor * m_dMinReward * ( 1 / ( 1
									// - m_dGamma ) );
	}
	 ...
	 * @return
	 */
	public double computeDiscountedRewardII(int cMaxStepsToGoal, PolicyStrategy policy,
			Vector<BeliefState> vObservedBeliefPoints, boolean bExplore, int[] aiActionCount) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		int iStep = 0, iAction = 0, iObservation = 0;
	...
	public double computeDiscountedRewardParticleFilteringImportanceSamplingII(int cMaxStepsToGoal,
			PolicyStrategy policy, int cParticles) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
	...
	public double computeDiscountedRewardParticleFilteringImportanceSampling(int cMaxStepsToGoal, PolicyStrategy policy,
			int cParticles) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		BeliefState bsCurrentBelief = getBeliefStateFactory().getInitialBeliefState(), bsNext = null;
				...
				for (iParticle = 0; iParticle < cUnfinishedParticles; iParticle++) {
					...
					if (vUnNormalizedWeighteds.elementAt(iStep)[iSample] > 0) {
						...
						if (isTerminalState(iNextState)) {
							vFinalWeights
									.add(vUnNormalizedWeighteds.elementAt(iStep)[iSample] * (dTruePr / dImportancePr));
							vFinalDiscountedRewards.add(vDiscountedRewards.elementAt(iStep - 1)[iSample]
									+ dDiscountFactor * dCurrentReward);
							vUnNormalizedWeighteds.elementAt(iStep + 1)[iParticle] = 0.0;
						} else {
							bDone = false;
							vUnNormalizedWeighteds
									.elementAt(iStep + 1)[iParticle] = vUnNormalizedWeighteds.elementAt(iStep)[iSample]
											* (dTruePr / dImportancePr);
						}
						...
						// vWeightTrajectories.elementAt( iStep + 1 )[iParticle]
						// = ( 1.0 / cParticles ) * dTruePr / dImportancePr;
						vStateTrajectories.elementAt(iStep + 1)[iParticle] = iNextState;
						vBeliefTrajectories.elementAt(iStep + 1)[iParticle] = bsNext;
						if (iStep > 0)
							vDiscountedRewards
									.elementAt(iStep)[iParticle] = vDiscountedRewards.elementAt(iStep - 1)[iSample]
											+ dDiscountFactor * dCurrentReward;
						else
	...
	/**
	 * Execute a single trial for computing discounted reward (DR) over the
	 ...
	 * @return
	 */
	public double computeMDPDiscountedReward(int cMaxStepsToGoal, MDPValueFunction policy, boolean bExplore,
			Vector<BeliefState> vBeliefPoints) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		...
		for (iStep = 0; (iStep < cMaxStepsToGoal) && !bDone; iStep++) {
			...
			if (false) {
				Logger.getInstance()
						.logln("Step " + iStep + ": s = " + getStateName(iState) + " a = " + getActionName(iAction)
								+ " o = " + getObservationName(iObservation) + " s' = " + getStateName(iNextState)
								+ " reward = " + dCurrentReward + " ADR = " + round(dDiscountedReward, 3)
			...
			if (vBeliefPoints != null) {
				...
				if (bsNext == null) {
					// Logger.getInstance().logln( bsCurrentBelief + " " +
					// iAction + " " + iObservation );
					// bsNext = bsCurrentBelief.nextBeliefState( iAction,
					// iObservation );
					return dDiscountedReward;
				}
	...
	public void setVisualisationUnit(VisualisationUnit vVisualisationUnit) {
